...
cox_cal = function(os_res, filename, gene_data) {
    ...
    write.csv(mulcox_res, paste0("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/cluster-an/cox/module_cluster_genes/subTME1/multi_cox_uni_res/", filename, ".csv"))
...
lapply(fs, function(x) {
    ...
    t = t[t$Pvalue <= 0.05, ]
    t = t[!is.na(t$Variable),]
    print(filename)
    if (dim(t)[1] != 0) {
      gene_data = as.data.frame(tcga_data[, which(colnames(tcga_data) %in% t$Variable)])
