...
all_cancers = read.csv("/public/home/yuwenqi/targers_work/data_concat/all_cancers.csv")
...
genes_needed = genes$gene[genes$gene %in% colnames(data)]

data = data[, genes_needed]
...
for (x in fs) {
    ...
    normal_data = normal_data[, genes$genes]
    temp_data = normal_data[,]
    normal_data['cancer_type'] = cancer_data[rownames(normal_data), 'cancer_type']
    plot_data = melt(normal_data, id.vars = c('cancer_type'))
    plot_data['cancer'] = c
    setwd("/public/home/yuwenqi/targers_work/data_process/exp_plot/")
    all_cancers_plot_data = rbind(all_cancers_plot_data, plot_data)
    exp_boxplot(plot_data, c)
    selected_data = temp_data[,colMeans(temp_data) < quantile(plot_data$value, 0.3)]
    setwd("/public/home/yuwenqi/targers_work/data_process/selected_data/")
...
all_exp_boxplot = function(plot_data) {
    p1 = ggplot(plot_data,aes(x=cancer,y=value),fill=cancer_type)+
        scale_fill_manual(values=c('#aed4e9','#db6968','#20B2AA',  '#F0F8FF',
                                    '#778899', '#EEA2AD', '#E0FFFF', '#CDCD00', 
                                    '#C2C2C2', '#C1CDC1', '#8E8E38', '#CD1076',
                                    '#AB82FF', '#FF1493', '#B03060', '#708090', 
        ...
        #                     label = "p.signif", method='wilcox.test', label.x = 0.55, label.y = max(plot_data['value']) - 0.15)  #+ stat_summary(aes(group=type), fun='mean', geom="point")
                         
    p1=p1+theme(legend.key = element_blank() ,legend.key.size = unit(0.2, "cm"), legend.title = element_blank(),
                panel.border = element_rect(linetype = 'solid', size = 0.3,fill = NA), text = element_text(size = 6) )
    ggsave(paste0("./", "all", ".pdf"), width = 15, height = 2.6, units = 'in')
}

plot_data = melt(data, id.vars = c('cancer_type', 'cancer'))

setwd("/public/home/yuwenqi/targers_work/data_concat/tumor_normal/")
all_exp_boxplot(plot_data)