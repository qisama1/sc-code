...
tcga_use = tcga.loc[meta.index, gene.CRC.dropna()]
tcga_use2 = tcga.loc[meta.index, gene2.CRC.dropna()]
...
for gene in tcga_use.columns.drop('type'):
    p = pd.DataFrame(tcga_use.loc[:, [gene]])
    p = pd.concat([p, os_res], axis=1)
    p.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/TCGA/CRC/os/" + gene + "_os.csv")

for gene in tcga_use2.columns.drop('type'):
    p = pd.DataFrame(tcga_use2.loc[:, [gene]])
    p = pd.concat([p, os_res], axis=1)
    p.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/TCGA/CRC/os_2/" + gene + "_os.csv")
```
