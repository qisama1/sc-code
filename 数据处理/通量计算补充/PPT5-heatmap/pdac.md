...
# Epi
major = meta_t.loc[meta.cluster == 'Epi'].index
t = sct.loc[major, module_epi.A]
t['ident'] = meta_t.loc[t.index, 'ident']
t.groupby('ident').mean().T.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_epi.csv")
# Mye
major = meta_t.loc[meta.cluster == 'Mye'].index
t = sct.loc[major, module_mye.A]
t['ident'] = meta_t.loc[t.index, 'ident']
t.groupby('ident').mean().T.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_mye.csv")
```
...
data = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_mye.csv", row.names=1)
module = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/modules_mye.csv")
rownames(data) = module$B
...
ggsave(filename = "/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_mye.pdf", heatmap, width = 12, height = 18, units = 'in')

data = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_epi.csv", row.names=1)
module = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/modules_epi.csv")
rownames(data) = module$B
heatmap = pheatmap(data, 
                   #annotation_row = annotation, # ?????????????????????????????????
                   #annotation_col=col, # ?????????????????????????????????
...
ggsave(filename = "/public/home/yuwenqi/sc-data/selected/append_ana/pathway_appends/PPT5-heatmap/pdac/pdca_heatmap_epi.pdf", heatmap, width = 12, height = 18, units = 'in')
```