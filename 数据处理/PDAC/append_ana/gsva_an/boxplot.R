...
data = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/gsva_an/pdac/plot_data.csv", row.names=1)
setwd("/public/home/yuwenqi/sc-data/selected/append_ana/gsva_an/pdac/")
...
p1 = ggplot(data,aes(x=subTME,y=Checkpoint.molecules),fill=subTME)+
    ...
    stat_compare_means(aes(group=subTME), symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", " ")),
                        label = "p.signif", method='kruskal.test', position = 'identity')  #+ stat_summary(aes(group=type), fun='mean', geom="point")
...
p1 = ggplot(data,aes(x=subTME,y=Anti.inflammatory.in.myeloid.cells),fill=subTME)+
    ...
    #coord_cartesian(ylim = c(0, 2))+
    stat_compare_means(aes(group=subTME), symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", " ")),
                        label = "p.signif", method='kruskal.test', label.x = 0.55)  #+ stat_summary(aes(group=type), fun='mean', geom="point")
