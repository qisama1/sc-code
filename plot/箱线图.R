...
p3 = ggplot(data,aes(x=type,y=GLUD2),fill=type)+
  scale_fill_manual(values=c('#FF1493', '#20B2AA'))+
  geom_boxplot(aes(fill=type),show.legend = F,trim = F)+
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
...
p4 = ggplot(data,aes(x=type,y=OGDH),fill=type)+
  ...
  #coord_cartesian(ylim = c(0, 2))+
  stat_compare_means(symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", " ")),
                     label = "p.signif", method='wilcox.test', label.y = 2.8, label.x = 0.5)
...
p5 = ggplot(data,aes(x=type,y=SUCLG1),fill=type)+
  ...
  stat_compare_means(symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", " ")),
                     label = "p.signif", method='wilcox.test', label.y = 2.3, label.x = 0.5)

plot_grid(p1, p2, p3, p4, p5, nrow=1)
ggsave("./all_genes_apoe_tumorcell.pdf", width = 6.5, height =3, device = cairo_pdf, units = 'in')
