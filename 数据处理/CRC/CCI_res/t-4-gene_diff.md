...
write.csv(as.matrix(scRNA[gene$Gene, ]@assays$RNA@data), "/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/gene_df.csv")

```

```python
data = pd.read_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/gene_df.csv", index_col=0).T
...
cellchat_clu = ['TGFβ_myCAF', 'Angiogenic_EC_1', 'wound_myCAF_1', 'ecm_myCAF', 'CD8Teff_2']
cellphonedb_clu = ['Macro_Pro', 'Macro_APOE', 'TGFβ_myCAF', 'ecm_myCAF', 'Angiogenic_EC_1']


cc = pd.read_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/cellchat_key.csv", index_col=0)
...
for i in cc.index:
    for col in cc.columns:
      if (cc.loc[i, col] > 0):
        ...
        if (cluster_1 not in cellchat_clu):
            continue
        ...
        p_t.loc[:, ['grade', 'node_type']] = grade.loc[p_t.index, ['grade', 'node_type']]
        p.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_a + "-" + cluster_1 + "_type.csv")
        p_t.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_a + "-" + cluster_1 + "_grade.csv")
        p = pd.DataFrame(data.loc[data.sub_cluster == cluster_2].groupby('sample').mean().loc[:, gene_b])
        p.loc[:, 'type'] = res.loc[p.index, 'type']
        p_t = p.loc[p.type == 'tumor']
        p_t.loc[:, ['grade', 'node_type']] = grade.loc[p_t.index, ['grade', 'node_type']]
        p.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_b + "-" + cluster_2 + "_type.csv")
        p_t.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_b + "-" + cluster_2 + "_grade.csv")
        if (flag):
            p = pd.DataFrame(data.loc[data.sub_cluster == cluster_2].groupby('sample').mean().loc[:, gene_c])
            p.loc[:, 'type'] = res.loc[p.index, 'type']
            p_t = p.loc[p.type == 'tumor']
            p_t.loc[:, ['grade', 'node_type']] = grade.loc[p_t.index, ['grade', 'node_type']]
            p.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_c + "-" + cluster_2 + "_type.csv")
            p_t.to_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellchat/" + gene_c + "-" + cluster_2 + "_grade.csv")
...
# 读取所有的文件
```R
setwd("/public/home/yuwenqi/sc-data/selected/CRC/workspace/module/module/cci_res/gene_diff/cellphonedb/")
...
lapply(fs, function(x) {
    ...
    filename = str_split(x,'[.;]',simplify = T)[,1]
    data = read.csv(x, row.names = 1)
    data=melt(data, id.vars=c('type', 'grade', 'node_type'))
...
lapply(fs, function(x) {
    ...
    data = read.csv(x, row.names = 1)
    data=melt(data, id.vars=c('type', 'grade', 'node_type'))
    boxplot_type2(data, paste0("./plot3/", filename, "_node_type.pdf"))
})
