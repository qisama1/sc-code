data = read.csv("/public/home/yuwenqi/sc-data/selected/9/module/percent_stage_compare/B.csv", row.names=1)
data=melt(data, id.vars=c("grade", 'type', 'stage'))
...
ggsave(filename = "./B_tumor_normal.pdf", plot = p1, device = 'pdf', width = 9, height = 3.5, units = 'in')



data = read.csv("/public/home/yuwenqi/sc-data/selected/9/module/percent_stage_compare/B_tumor.csv", row.names=1)
data=melt(data, id.vars=c("grade", 'type', 'stage'))
...
ggsave(filename = "./B_stage.pdf", plot = p1, device = 'pdf', width = 9, height = 3.5, units = 'in')
...
data$grade <- factor(data$grade,levels = c("I","II", "III"))

bioCol=c("#1c79c0","#ff3399", "#000000")
...
ggsave(filename = "./B_grade.pdf", plot = p1, device = 'pdf', width = 9, height = 3.5, units = 'in')
