data_risk = read.csv("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/cluster-an/cox/module_cluster_genes/subTME1/risk2/all_risk.csv", row.names=1)
...
res <- coxph(Surv(days, os) ~ age  + risk, data =  data)
mul_cox <- summary(res)
...
#画图
setwd("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/cluster-an/cox/module_cluster_genes/subTME1/forestplot/")
pdf("./riskscore_age.pdf",onefile=FALSE, width = 5, height = 2)
forestplot(dat[,c(1,5,6)], #显示表格的第1，5，6列内容
           ...
           hrzl_lines=list("1" = gpar(lty=1,lwd=1.5),
                           ...
                           "4"= gpar(lty=1,lwd=1.5)), # 在1,2,7行添加横线
