...
tcga_data = read.csv("/public/home/yuwenqi/sc-data/selected/new-RNAseq/new_process/split_data/ESCA_data.csv", row.names=1)
os_res = read.csv("/public/home/yuwenqi/sc-data/selected/new-RNAseq/ESCA/os/os_meta.csv")

os_res$days_to_last_followup[is.na(os_res$days_to_last_followup)] = 0 #is.na()??????????????????????????????
...
tcga_data = tcga_data[os_res$X, ]
lasso_cal = function(os_res, filename, unicox) {
    unicox = unicox[unicox$Pvalue <= 0.05, ]
    if (dim(unicox[1]) > 1) {
        X <- as.matrix(tcga_data[os_res$X, which(colnames(tcga_data) %in% unicox$Variable)])
        Y <- data.matrix(Surv(os_res$days,os_res$os))
        
        f1 = glmnet(X, Y, family="cox", nlambda=100, alpha=1)
        
        pdf(paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/lasso/" , filename, "_" ,"lambda_plot_f1.pdf"),width=15,height=15)
        plot(f1, xvar="lambda",label=TRUE)
        dev.off()

        pdf(paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/lasso/" , filename, "_" ,"lambda_plot_cv.pdf"),width=15,height = 15)
        ...
        write.csv(genes, paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/lasso/" , filename, "_" ,"genes.csv"))
        X <- as.data.frame(X)
        Y <- as.data.frame(Y)
        ...
        res_path = paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/risk2/", filename, "_multicox.csv")
        ...
        if (k != 0) {
            ...
            res_path = paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/risk2/", filename, "_risk.csv")
...
setwd("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/cox_res/")
