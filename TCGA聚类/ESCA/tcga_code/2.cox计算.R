library("survival")
#以R包survival中的肺癌数据为例
data = read.csv("/public/home/yuwenqi/sc-data/selected/new-RNAseq/new_process/split_data/ESCA_data.csv", row.names=1)
...
cox_cal = function(os_res, filename, gene_data) {
    ...
    write.csv(Unicox, paste0("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/cox_res/", filename, ".csv"))
}



setwd("/public/home/yuwenqi/sc-data/selected/9/cluster-an/cox/module_cluster_genes/subTME2/")
fs = list.files('./', 'csv')
lapply(fs, function(x) {
    filename = str_split(x,'[.;]',simplify = T)[,1]
    genes = read.csv(x, row.names=1)
    gene_data = data[os_res$X, which(colnames(data) %in% genes$gene)]
    cox_cal(os_res, filename, gene_data[os_res$X, ])
})