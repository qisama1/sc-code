setwd("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/BC3-CCI/module/module/cci_an_sheet/pathway_corr/gsva_diff/gsva_end/")
data = read.csv("./gsva_end.csv")
p1 = ggplot(data,aes(x=ident,y=interferon.EC),fill=ident)+
...
ggsave("./gsva_epi_interferon_EC.pdf", width = 4, height =4, device = cairo_pdf, units = 'in')

p1 = ggplot(data,aes(x=ident,y=angiogenic.EC),fill=ident)+
  scale_fill_manual(values=c('#ff890f','#7CFC00','#20B2AA',  '#F0F8FF',
                             '#778899', '#EEA2AD', '#E0FFFF', '#CDCD00', 
                             '#C2C2C2', '#C1CDC1', '#8E8E38', '#CD1076',
                             '#AB82FF', '#FF1493', '#B03060', '#708090', 
  ...
  stat_compare_means(symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", " ")),
                     label = "p.signif", method='kruskal.test', label.x = 0.5)  + stat_summary(fun='mean', geom="point")
ggsave("./gsva_epi_angiogenic_EC.pdf", width = 4, height =4, device = cairo_pdf, units = 'in')
