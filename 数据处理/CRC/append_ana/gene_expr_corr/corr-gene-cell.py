data = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/gene_exp/CRC/gene_df.csv", index_col = 0).T
...
data['orig.ident'] = meta['orig.ident']
...
for i in part1_gene.index:
    ...
    if (g not in data.columns) :
        continue
    if (v.type == 'TAM'):
        major = meta.loc[meta.loc[meta.sub_cluster.str.contains('Macro')].index,]
        d = data.loc[major.index, ].groupby('orig.ident').mean().loc[:, [g]]
        res.loc[d.index, v.type + "_" + v.gene] = d[g]
    if (v.type == 'CD8+T'):
        major = meta.loc[meta.loc[meta.sub_cluster.str.contains('CD8')].index,]
        d = data.loc[major.index, ].groupby('orig.ident').mean().loc[:, [g]]
...
corr_res = pd.DataFrame(index = res.columns, columns = part1_percent.loc[part1_percent.type == 'CRC'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/CRC/part1_corr.csv")
...
corr_res = pd.DataFrame(index = res.columns, columns = part2_percent.loc[part2_percent.type == 'CRC'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/CRC/part2_corr.csv")
...
corr_res = pd.DataFrame(index = res.columns, columns = part3_percent.loc[part3_percent.type == 'CRC'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/CRC/part3_corr.csv")