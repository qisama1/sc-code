mye = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/mye_info.csv", index_col=0)
fib = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/fib_info.csv", index_col=0)
end = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/end_info.csv", index_col=0)
b = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/b_info.csv", index_col=0)
tnk = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/tnk_info.csv", index_col=0)
epi = pd.read_csv("/public/home/yuwenqi/sc-data/selected/31/epi_info.csv", index_col=0)
...
meta[meta.Sample2_Origin.isin(['nLung', 'tLung'])]

meta = pd.concat([mye, fib, end, b, tnk, epi])
meta_use = meta[meta.type.isin(['Tumor', 'Normal'])]
meta = meta_use
# 算所有样本的所有亚类属于其大类的细胞比例
num2 = meta.groupby([ 'cluster','Sample2']).count().iloc[:, 0]
t = pd.DataFrame()
for i in set(meta.ident):
    x = meta[meta['ident'] == i]
    num1 = x.groupby('Sample2').count().iloc[:, 0]
...
# 算大类细胞比例
num2 = meta.groupby('Sample2').count().iloc[:, 0]
t = pd.DataFrame()
