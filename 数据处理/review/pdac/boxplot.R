...
setwd("/public/home/yuwenqi/sc-data/selected/review/pdac/tumor_normal_boxplot/")
fs = list.files('./', 'csv')
lapply(fs, function(x) {
    ...
    p1=p1+theme(legend.background = element_blank(), legend.key = element_blank() ,legend.key.size = unit(0.5, "cm"), legend.title = element_blank(),
                panel.border = element_rect(linetype = 'solid', size = 0.3,fill = NA), text = element_text(size = 14) )
                   
    ggsave(paste0("/public/home/yuwenqi/sc-data/selected/review/pdac/tumor_normal_boxplot/plot/",filename, ".pdf"), width = 10, height = 2.6, units = 'in')
