```py
sub1 = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/sub1.csv")
sub2 = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/sub2.csv")
sub3 = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/sub3.csv")
genes = set()
# 得到需要的基因
...
pd.DataFrame(genes, columns = ['gene']).to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/gene_needed.csv")
# 构建结果列表
cols = []
for sub in [sub1, sub2, sub3]:
...
# 构建结果集，应该构建三个
data = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/gene_df.csv", index_col = 0).T
...
cellchat = pd.read_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/CRC1/cellchat/module2_df_net_v.csv", index_col = 0)
for i in res2.index:
...
for idx in res3.index:
    for col in res3.columns:
        if (len(idx.split('_')) > 1):
            ...
            cluster1 = col.split('|')[0]
            cluster2 = col.split("|")[1]
            res3.loc[idx, col] = (data.loc[meta.loc[meta.sub_cluster == cluster1].index, gene1].mean() + data.loc[meta.loc[meta.sub_cluster == cluster2].index, gene2].mean() + data.loc[meta.loc[meta.sub_cluster == cluster2].index, gene3].mean()) / 3
        else:
            gene1 = idx.split('|')[0]
            gene2 = idx.split("|")[1].split('_')[0]
            cluster1 = col.split('|')[0]
            cluster2 = col.split("|")[1]
            res3.loc[idx, col] = (data.loc[meta.loc[meta.sub_cluster == cluster1].index, gene1].mean() + data.loc[meta.loc[meta.sub_cluster == cluster2].index, gene2].mean()) / 2
cellchat = pd.read_csv("/public/home/yuwenqi/sc-data/selected/CRC/workspace/CRC1/cellchat/module3_df_net_v.csv", index_col = 0)
for i in res3.index:
...
pd.concat([res1.loc[sorted_idx, res1.sum().sort_values(ascending=False).index], res2.loc[sorted_idx, res2.sum().sort_values(ascending=False).index], res3.loc[sorted_idx, res3.sum().sort_values(ascending=False).index]], axis=1).to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/bubble_base.csv")
```

```R
scRNA = qread("/public/home/yuwenqi/sc-data/selected/CRC/workspace/CRC1/crc1_used.qs")
gene = read.csv("/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/gene_needed.csv")
write.csv(as.matrix(scRNA[gene$gene, ]@assays$RNA@data), "/public/home/yuwenqi/sc-data/selected/append_ana/cci-bubble/CRC/cellchat/gene_df.csv")
```
```