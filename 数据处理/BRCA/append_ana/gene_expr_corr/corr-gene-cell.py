data = pd.read_csv("/public/home/yuwenqi/sc-data/selected/append_ana/gene_exp/BC/gene_df.csv", index_col = 0).T
...
corr_res = pd.DataFrame(index = res.columns, columns = part1_percent.loc[part1_percent.type == 'BRCA'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/BC/part1_corr.csv")
...
corr_res = pd.DataFrame(index = res.columns, columns = part2_percent.loc[part2_percent.type == 'BRCA'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/BC/part2_corr.csv")
...
corr_res = pd.DataFrame(index = res.columns, columns = part3_percent.loc[part3_percent.type == 'BRCA'].cluster)
for i in corr_res.index:
    for j in corr_res.columns:
        if (spearmanr(t[j], res[i])[1] < 0.05):
            corr_res.loc[i, j] = spearmanr(t[j], res[i])[0]
corr_res.to_csv("/public/home/yuwenqi/sc-data/selected/append_ana/corr-gene-cell/BC/part3_corr.csv")