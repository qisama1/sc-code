...
fib_res.index = fib_res.index.str.replace('-', '_')
...
for cluster in module.module4.dropna():
    flag = True
    if cluster in end_res.index:
        flag = False
        idx = end_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, end_marker.loc[end_marker.cluster == idx]])
        end_marker.loc[end_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/" + cluster + ".csv")
    if cluster in epi_res.index:
        flag = False
        idx = epi_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, epi_marker.loc[epi_marker.cluster == idx]])
        epi_marker.loc[epi_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/" + cluster + ".csv")
    if cluster in fib_res.index:
        flag = False
        idx = fib_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, fib_marker.loc[fib_marker.cluster == idx]])
        fib_marker.loc[fib_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/" + cluster + ".csv")
    if cluster in tnk_res.index:
        flag = False
        idx = tnk_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, tnk_marker.loc[tnk_marker.cluster == idx]])
        tnk_marker.loc[tnk_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/" + cluster + ".csv")
    if cluster in mye_res.index:
        flag = False
        idx = mye_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, mye_marker.loc[mye_marker.cluster == idx]])
        mye_marker.loc[mye_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/"  + cluster + ".csv")
    if cluster in b_res.index:
        flag = False
        idx = b_res.loc[cluster, 'cluster']
        genes = pd.concat([genes, b_marker.loc[b_marker.cluster == idx]])
        b_marker.loc[b_marker.cluster == idx].to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/" + cluster + ".csv")
    if flag :
        print(cluster)

genes = genes.loc[~genes.index.duplicated()]
genes.to_csv("/public/home/yuwenqi/sc-data/selected/35/cluster-an/cox/module_cluster_genes/subTME4/"  + "all" + ".csv")
```