pbmc = qread("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/Fib/fib.qs")

## Runing in r-4.1.3
setwd("/public/home/yuwenqi/sc-data/selected/Breast/workspace2/monocle/Fib/")
...
# p + geom_vline(xintercept = seq(-2.5,-6,0.25)) + geom_hline(yintercept = seq(0,1,0.25))
embed <- data.frame(Embeddings(pbmc, reduction = "umap"))
embed <- subset(embed, UMAP_1 > 13 & UMAP_1 < 13.5 & UMAP_2 > -5.5 & UMAP_2 < -5)
root.cell <- rownames(embed)
cds <- order_cells(cds, root_cells = root.cell)
pdf("./pseudotime-iFNy_iCAF2.pdf")
